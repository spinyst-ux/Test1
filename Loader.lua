...
-- Case-insensitive, whitespace-trimmed name compare so a stray space or different capitalization
-- in the "Join Player Name" box doesn't stop follow-host from recognizing the real host.
...
-- does NOT touch isAutoplay: handleLobbyAutomation (gated on isAutoplay) is what re-sends the
-- join/follow request once we're actually back in the lobby, so autoplay must stay on across cycles.
local function leaveDungeonForHostLeave()
if hasReturnedToLobby then return end
hasReturnedToLobby = true
if humanoid then
humanoid.AutoRotate = true
...
task.spawn(function()
    ...
    if SETTINGS.FollowHost or SETTINGS.LobbyMode == "Join" then
        if normalizeName(SETTINGS.JoinPlayerName) ~= "" then
            local hostName = normalizeName(SETTINGS.JoinPlayerName)
            local hostInServer = false
            for _, p in ipairs(Players:GetPlayers()) do
                if normalizeName(p.Name) == hostName or normalizeName(p.DisplayName) == hostName then
                    hostInServer = true
                    break
                end
            end
...
MakeSectionLabel("Performance (auto dodge still sees every attack)", miscPage).LayoutOrder = 105
UI.removeMapRow = MakeToggle("Remove Map: " .. (SETTINGS.RemoveMap and "ON" or "OFF"), SETTINGS.RemoveMap, miscPage)
UI.removeMapRow.LayoutOrder = 106
UI.fpsBoostRow = MakeToggle("FPS Boost: " .. (SETTINGS.FpsBoost and "ON" or "OFF"), SETTINGS.FpsBoost, miscPage)
UI.fpsBoostRow.LayoutOrder = 107
...
UI.removeMapRow.MouseButton1Click:Connect(function()
...
runMacroBtn.MouseButton1Click:Connect(function()
    ...
    if isAutoplay then
        -- Restarting Autoplay without a character respawn (e.g. Stop then Start again mid-dungeon) used to leave
        -- these stuck from the previous run, silently disabling ReplayTime / stage-loss webhook / party-disconnect
...
function UI.refreshPerfRows()
    for _, r in ipairs({ { UI.removeMapRow, "Remove Map: ", SETTINGS.RemoveMap }, { UI.fpsBoostRow, "FPS Boost: ", SETTINGS.FpsBoost } }) do
        if r[1] then
...
function UI.applyRemoveMap(enabled, skipSave)
    ...
    if not skipSave then
        saveConfig()
        setStatus(SETTINGS.RemoveMap and "Status: Remove Map ON" or "Status: Remove Map OFF", true)
    end
end
...
function UI.applyFpsBoost(enabled, skipSave)
    ...
    if not skipSave then
        saveConfig()
        setStatus(SETTINGS.FpsBoost and "Status: FPS Boost ON" or "Status: FPS Boost OFF", true)
    end
end
...
if isInLobby() then
    ...
    UI.hostSeen, UI.hostMissingSince = false, nil
    -- Reset every per-dungeon flag here, not just on CharacterAdded: rejoining the same server's
    -- lobby (e.g. after leaveDungeonForHostLeave) doesn't always respawn the character, and without
    -- this reset hasReturnedToLobby stays stuck true forever, which permanently blocks
...
-- Still leaving the dungeon (leaveDungeonForHostLeave/returnToLobby fired but isInLobby() hasn't
-- flipped true yet): hold everything below off, including the stage-loss/follow-host checks, so
...
-- FOLLOW HOST: once the host has been in this dungeon, leave to the lobby 3 s after they disappear
-- from the server. Applies whether this client's own LobbyMode is "Host" or "Join" -- FollowHost is
-- an independent toggle from the lobby-role setting. The host is always the literal, user-typed
-- SETTINGS.JoinPlayerName: never substitute "whoever is currently in the server" (e.g. a party
-- member Roblox happens to migrate/replicate in as the server briefly reshuffles) as the real host.
if SETTINGS.FollowHost and normalizeName(SETTINGS.JoinPlayerName) ~= "" then
    local hostName = normalizeName(SETTINGS.JoinPlayerName)
    local hostHere = false
    for _, p in ipairs(Players:GetPlayers()) do
        if normalizeName(p.Name) == hostName or normalizeName(p.DisplayName) == hostName then hostHere = true; break end
    end
    if hostHere then
...
local function realClickButton(btn, attempt)
    ...
    elseif attempt == 3 then
        return pcall(function()
            ...
            if not mouse1click then error("executor mouse click unavailable") end
...
function UI.applyBuild(statName, buildName)
    ...
    task.spawn(function()
        local ok, err = pcall(function()
            ...
            local startPoints, last, stall = points, points, 0
            while points > 0 and not isCleaningUp do
                ...
                last = points
                if stall >= 1 and stall <= 3 then realClickButton(btn, stall) end
                if stall >= 5 then break end
...
-- Toggle: Required = false -> key system OFF (free/keyless). Required = true -> key system ON (needs valid key).
local KEY = {
    Required = false, -- false = key system off, true = key system on
